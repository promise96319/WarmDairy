...
public class SQTextEditorView: UIView {
    ...
    private enum JSFunctionType {
        ...
        var name: String {
            ...
            case .setPlaceholder(let text):
                return "setPlaceholderText('\(text)')"
    ...
    private lazy var webView : RichEditorWebView = {
    ...
    public func setTextAlignment(alignment: String, completion: @escaping (_ error: Error?) -> ()) {
        selectedTextAttribute.align = QinTextAlign(rawValue: alignment)!
        webView.evaluateJavaScript(JSFunctionType.setText(alignment: alignment).name, completionHandler: { (value, error) in
               completion(error)
           })
       }
    
    public func setPlaceholder(text: String, completion: @escaping (_ error: Error?) -> ()) {
       webView.evaluateJavaScript(JSFunctionType.setPlaceholder(text: text).name, completionHandler: { (value, error) in
            completion(error)
        })
    }
